...
- [x] `SC-010 | discovery, operations, observability, platform-persistence |` Completar el hardening del canvas React Flow: `focusedServiceId` como fuente de verdad para la seleccion visual, `dragHandle` real en el header, acciones runtime blindadas con `nodrag`/`nopan` y contexto del inspector estable aunque el usuario haga click en el pane.
...
- [x] `SC-030 | discovery, operations, observability, platform-persistence |` Derivar la topologia visual desde manifests `/internal/topology` con fallback local `topology.manifest.json`, store/poller central, edges HTTP/RabbitMQ, port hints StylePlus (`3000-3014`) y feature flag `TOPOLOGY_SOURCE=manual|manifest|hybrid` con default `hybrid`.
- [x] `SC-031 | discovery, operations, observability |` Segmentar el inspector derecho en tabs reales por tipo de informacion (`Overview`, `Topology`, `Logs`, `Alerts`, `Events`, `k6`) para evitar saturacion visual, mover las acciones runtime al overview, aislar topology en su propia superficie y dejar que solo una capa del rail haga scroll a la vez.
- [x] `SC-032 | discovery, operations, observability |` Volver manual la resincronizacion de topology manifests: quitar el polling y los refresh implicitos por cambios de runtime o acciones operativas, dejando `Refresh topology` como disparador explicito desde el canvas.
